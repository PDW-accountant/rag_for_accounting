...
검증 항목:
  ...
  - 조항 단위(clause-level) 분할 — clause_level=True일 때 "#### N.N" 헤더 경계로 조항별 청크를
    나누고, 머리말은 별도 조각으로 보존하며, 하위 헤더(#####)는 분할 경계로 쓰지 않는다
...
from src.ingest.ontology.models import OntologyGraph, OntologyNode
