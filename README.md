...
이 저장소 자체가 플러그인입니다(`src/.codex-plugin/plugin.json`). MCP 서버와 달리 Codex는 플러그인을 마켓플레이스(marketplace: 플러그인을 등록해두는 로컬 소스) 경유로 설치합니다.
...
> MCP 서버(`src/mcp/server.py`)는 플러그인에 번들되어 있어 위 설치만으로 함께 등록됩니다 — Claude처럼 `mcp add`를 별도로 실행할 필요가 없습니다. DB·임베딩 서버가 떠 있어야 합니다(`docker compose up -d database embedding`).
...
flowchart TD
    ...
    EVAL -->|근거 부족, 재시도 가능<br/>최대 3회| REWRITE
